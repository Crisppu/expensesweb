...
import { Sizes as fontSize } from "../../assets/theme/theme-rtl";

interface SoftInputProps extends React.ComponentProps<typeof InputBase> {
    ownerState: {
        ...
        success: boolean;
        disabled: boolean;
        iconDirection?: 'left' | 'right'| "none" | string;
        direction?: 'left' | 'right'| "none" | string;
    };

}
export const SoftInputRoot= styled(InputBase)<SoftInputProps>(({ownerState, theme }) =>{
    const { palette, boxShadows, functions, borders } = theme;
    const { size,error,success,disabled,iconDirection,direction } = ownerState;
